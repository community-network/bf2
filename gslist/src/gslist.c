...
#define set_custom_multi_query(X) \
                if((argv[i][0] >= '0') && (argv[i][0] <= '9')) { \
                    ...
                    if (!quiet) fprintf(stderr, "- hex dump of the packet to sent:\n\n"); \
...
handle_servers:
    ...
    while(len >= 6) {
        ...
        if(execstring) {
          ...
          if (!quiet) fprintf(stderr, "   Execute: \"%s\"\n", tmpexec);
            system(tmpexec);
        }
    ...
    if((megaquery >= 0) || ((megaquery == -2) && multi_query_custom_binary)) {
        if (!quiet) fprintf(stderr, "- querying servers:\n");
...
void lame_room_visualization(u8 *buff) {
    ...
    if (!quiet) fprintf(fdout, "#GPG!%-5d %s\n", (n1 << 24) | (n2 << 16) | (n3 << 8) | n4, r);
}
...
u8 *gslist_input_list(u8 *fdlist, int *len, u8 *buff, int *dynsz) {
    ...
    if (!quiet) fprintf(stderr, "- open file %s\n", fdlist);
    fd = fopen(fdlist, "rb");
    if(!fd) std_err();
